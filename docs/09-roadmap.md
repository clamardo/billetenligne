...
774 tests · 181 smoke checks · 32 executed schema guarantees · 159 further tests against real Postgres, and 10 against real Azurite.
...
- ⬜ **Disruption — the re-accommodation plan** (`08-disruption.md` §2.2 onwards) — the larger half, and the one that moves people rather than informing them: ranked options, seats remapped onto a different layout, protection on another operator, the passenger's own choice, and the atomic rebooking wave. Everything it needs now exists — a departure whose state everybody agrees on, and passengers who already know

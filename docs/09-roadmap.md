...
- ⬜ **Disruption — the passenger's own choice** (`08-disruption.md` §3.2 option ⑤) — letting the passengers choose between the plans themselves instead of a dispatcher choosing for them, which §3.2 argues produces better coverage than any dispatcher plan because a released seat returns to the pool
...
The *payment path* is built end to end and waiting on a telco. The phase is not: the reconciliation console exists, disruption is declared, told and recorded, a rescue coach can be sent, the passengers of a broken coach can be moved onto the next departure, money leaves under two-person control, and a rail is enabled by pushing a file — and a passenger whose coach failed can be carried by another company under an agreement signed months earlier — but the passenger's own choice (§3.2 option ⑤) and the payout statement PDF are unbuilt. Saying "everything is built" here — as an earlier draft of this section did — is exactly the kind of claim this document exists to refuse.

...
**Updated:** 2026-08-13 · after commit *Standing in Dolisie, asking for Pointe-Noire, and being answered*
...
dart test packages/bel_domain packages/bel_localization \
         packages/bel_contracts packages/bel_crypto     # 585 tests
...
cd apps/traveller && flutter test        # 225 app tests, incl. real SQLite
...
./infra/migrations/check.sh              # 41 schema guarantees
./tool/integration.sh                    # 457 tests on real Postgres, incl. the worker
./tool/smoke_api.sh                      # 390 checks, incl. the Dart client
...
**1,337 tests in total**, plus 390 smoke checks, 41 executed schema guarantees,
457 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

...
dart test packages/bel_domain packages/bel_localization \
         packages/bel_contracts packages/bel_crypto     # 588 tests
...
dart run tool/check_layers.dart          # the onion rule, 388 files
./infra/migrations/check.sh              # 42 schema guarantees
./tool/integration.sh                    # 464 tests on real Postgres, incl. the worker
./tool/smoke_api.sh                      # 392 checks, incl. the Dart client
...
**1,346 tests in total**, plus 392 smoke checks, 42 executed schema guarantees,
464 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

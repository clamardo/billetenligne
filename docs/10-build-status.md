...
dart test services/api -x integration -x storage        # 258 tests
...
cd apps/traveller && flutter test        # 226 app tests, incl. real SQLite
cd apps/console   && flutter test        # 123 console tests
...
dart run tool/check_layers.dart          # the onion rule, 399 files
./infra/migrations/check.sh              # 43 schema guarantees
./tool/integration.sh                    # 484 tests on real Postgres, incl. the worker
./tool/smoke_api.sh                      # 427 checks, incl. the Dart client
...
**1,374 tests in total**, plus 427 smoke checks, 43 executed schema guarantees,
484 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

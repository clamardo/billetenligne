...
dart test services/api -x integration -x storage        # 249 tests
...
dart run tool/check_layers.dart          # the onion rule, 397 files
./infra/migrations/check.sh              # 43 schema guarantees
./tool/integration.sh                    # 482 tests on real Postgres, incl. the worker
...
**1,365 tests in total**, plus 427 smoke checks, 43 executed schema guarantees,
482 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

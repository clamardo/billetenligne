...
dart run tool/check_layers.dart          # the onion rule, 387 files
./infra/migrations/check.sh              # 40 schema guarantees
./tool/integration.sh                    # 444 tests on real Postgres, incl. the worker
./tool/smoke_api.sh                      # 384 checks, incl. the Dart client
...
**1,336 tests in total**, plus 384 smoke checks, 40 executed schema guarantees,
444 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

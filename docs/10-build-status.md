...
dart run tool/check_layers.dart          # the onion rule, 394 files
./infra/migrations/check.sh              # 43 schema guarantees
./tool/integration.sh                    # 480 tests on real Postgres, incl. the worker
./tool/smoke_api.sh                      # 418 checks, incl. the Dart client
...
**1,352 tests in total**, plus 418 smoke checks, 43 executed schema guarantees,
480 further tests against real Postgres and 10 against real Azurite. The smoke run now includes the *typed client* against the running

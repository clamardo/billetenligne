...
| **The seat-layout section builder** | ✅ done | The coach no preset fits, drawn section by section, with `KSeatMap` — the widget that sells the seat — redrawn on every keystroke. Start rows are computed, not typed. `Abreast` in the domain now parses to null instead of throwing, which turned three 500s into field-named 400s. Seats are taken out of service by touching them, doors and lavatories are placed, and sections reorder without being retyped. 8 domain · 9 contract · 14 widget tests, 1 on real Postgres, and 15 smoke checks on a real socket |
...
| **Platform subscription billing** | ✅ done | `04-payments.md` §6.2. The other direction: what an operator owes the platform, not what the platform owes them. `PlatformBillingRail` mirrors the mobile-money port exactly — `card` (a real Stripe Checkout request, one class from a live key) and `bank_transfer` (a settlement account out of config, no network call ever) — behind a get-or-create keyed on `(operator_id, period_start)`, so a second ask the same month accrues nothing twice. Posts to a new `revenue:subscription` ledger line, opposite `receivable:operator:<id>`, kept off the payout statement screen on purpose — the two numbers never net against each other. One flat tier, no plan-selection UI. New `billing.manage` capability, bundled into `finance` · `org_admin` · `org_owner`; a console screen shows amount due, a payment-type picker, and the matching instructions. Migration 0047 grants `bel_app`/`bel_admin` from its first line — the wallet-verification gap, checked this time by a real-Postgres integration test asking as `bel_app`. 22 domain · 5 Postgres · 162 console tests |
...
| **Segment selling — a seat's occupancy is a range** | ✅ done | ADR-0025. `seats.state` is a scalar and *sold Brazzaville→Dolisie, free Dolisie→Pointe-Noire* is one row with two answers, so occupancy moved into its own table as a **half-open range of stop positions** with an **EXCLUDE constraint** doing the work a handler cannot: two people cannot buy the same piece of the same seat, refused by Postgres under concurrency rather than by a review. `[0,1)` and `[1,2)` share nothing, which is a passenger alighting at Dolisie and another boarding there. **The whole road is a range too**, so the commonest sale takes the same path as a segment. `seats.state` is now **derived by trigger** and gains `partial`, so an untaught reader fails closed. Positions are captured on the departure, so editing a road cannot renumber what was sold. **A segment is sellable only when priced** — no pro-rata invention — which is what leaves today's behaviour untouched until an operator prices one. The console now has that price list: a table of legs on the route form, **cities on the wire and positions in the database**, with the road's rules applied to the price — a set-down-only stop cannot start a leg, the whole road is refused because the timetable already prices it, and a leg priced at nothing is refused where the person who typed it is standing. The currency is the market's, never the client's. Every path that takes a seat now writes occupancy — claim, release, both sweepers, reservation, capture, self-cancellation, refund, passenger choice, reschedule, paid change, rescue coach, protection movement: twenty-two sites across nine files. **The grant is the control**: no application role holds `UPDATE` on `seats`, so a path added next year that sets the state directly fails in the suite rather than in production, and the seat race is the exclusion constraint rather than a lock somebody remembered to take. A sale **re-attributes the hold's own row**, so a paid seat is never briefly free; an unpaid reservation stays under its hold, which is what makes it read as held; and a claim that loses the race takes its own hold down with it, so no traveller's retry is answered with "that key is spent". A seat map meeting a state it has never heard of now says *held*, never *available*. **Search now finds a leg**: a traveller in Dolisie asking for Pointe-Noire gets the coaches that pass through, as ordinary departures between those two towns — the operator's price, the boarding time at the stop, the arrival at the stop, and only the towns in between. No new DTO and no second screen, so a client that has never heard of segments renders it right. Availability asks occupancy rather than `seats.state`, because a seat sold Brazzaville→Dolisie is exactly what somebody boarding at Dolisie should be offered. **The seat map answers for the leg too**, asked with the same pair the search was made with — so a seat sold Brazzaville→Dolisie is drawn as free to somebody boarding at Dolisie, priced at the operator's price for that leg, flat across the coach. A pair nobody priced has no seat map rather than an empty one. **And the traveller buys it**: the claim resolves the same pair against the operator's price list inside the transaction and occupies that piece of road alone, so one seat carries a passenger to Dolisie and another from Dolisie on the same run. A pair nobody priced is refused by name, never sold as the whole road. The hold remembers what it was quoted — a price list edited between the tap and the counter cannot rewrite what somebody agreed to pay — and `bel_public`'s table-wide UPDATE on `holds` became the one column it always was, because a price on that row made the old grant a one-franc ticket. **The conductor's list names it**: a manifest row carries where that passenger gets on and off, and only when it is a piece of the road — the header already names the whole road, and a list where every line is annotated is a list nobody reads the annotations on. That is the one place somebody finds out 12A comes free at Dolisie. **And the ticket names the journey somebody bought**: the leg's two towns, the hour the coach reaches the first of them — minutes from the departure, so a delay carries the boarding time with it — the terminus arrival unchanged, and the stop's own yard, or none at all rather than the terminal in Brazzaville the coach left from. A whole-road booking reads exactly as it always did. **And the door knows**: the scanner's verdict says *Descend à DOL* under the name, and the manual list carries the leg beside the reference. It rides on the pinned manifest rather than inside the signed QR — a format change would have every un-updated scanner refusing every ticket issued after it, for a fact the device downloaded before the coach left. On the refusals too, since the verdict a conductor argues with is the one that says no. It reaches the door over the wire now (ADR-0022), and through the demo manifest on a clone with no server — the same mapping either way. 19 domain · 6 contract · 24 Postgres · 4 console · 4 traveller · 5 scanner · 18 smoke · 4 schema guarantees |
...
| **The ticket on paper** | ✅ done | [ADR-0034](adr/0034-the-ticket-on-paper.md). ADR-0026 gave the walk-in a ticket they can *reach*; it assumed they have somewhere to receive a message. The most common sale in this market is cash across a counter to somebody with a feature phone, and the guichet's unverified account has **no reachable channel at all** — the ticket was issued, signed, and unreachable. What kept this shut was **one wrong sentence in the codebase**: the console receipt's doc comment said a printed code "would be a frozen code that fails the check (ADR-0007)". It would not. `tickets.payload` carries **no freshness code** — the control at the door is one scan per seat, recorded on the manifest — and the rotating six digits defend only against a photograph of a *live screen*, which a printed ticket presents none of. Nothing else in the repo agreed with that sentence; it was simply the only prose anybody would read before deciding, and it said no. So: **`/b/{token}?format=boardingPass|a4`**, the same URL the customer already holds, rendered by `PrintedTicketPage`. The default is a DL pass **three to an A4 sheet with a cut line**, because "boarding pass" normally implies a thermal printer and rolls of card while an agency in Dolisie has an inkjet and a ream of copier paper; `a4` gives one per sheet for the counter that hands over a whole page. The choice is made at the printer by the vendor, not stored as a setting. **The design is the operator's and it is data** — `TicketDesign` in `bel_domain`: a format, one of the eight verified hues, one of four motifs, their mark, which optional lines to print, and a sentence of their own at the foot; four starters ship as domain constants (`classicPass`, `economyPass`, `fullPageReceipt`, `largePrintPage`) and `ticket_designs` stores the rest as JSONB behind `FORCE ROW LEVEL SECURITY` with one default per format by partial unique index. **Bounded for physical reasons**, harder than the vitrine is: a free colour picker eventually prints a hue invisible in direct sun or a dark ground a cheap scanner refuses, and a free layout eventually moves the QR under the fold. **Tolerant on read, canonical on write** — `fromJson` drops what it does not recognise and never throws, because a counter that cannot sell because a JSON document gained a field is a counter turning customers away. **Nothing on the page is fetched**: no stylesheet, no webfont, no `<img>`, no script — a font that arrives after the print dialog lays out the wrong widths and a 404 leaves a white hole where the company's mark should be, on paper, where nobody can reload it. The mark is a drawn monogram, the weave a `data:` URI, the QR inline SVG. Colour **is** printed here, unlike ADR-0026's own boarding pass: this is the operator's stationery and they asked for their header on it; the operator who would rather spend no ink picks `economyPass`, which has none to spend. **And the till can print.** `POST /console/v1/bookings/{ref}/print` is the one place a token reaches a console screen, and the exception is made as small as it goes — **twenty minutes**, its own `print` channel, `sent_to` recording the till rather than an address, never sent anywhere, and the customer's own emailed link left alive. `queueSend` still refuses to return one, for the reason it always did. **The guard that matters** is not that the payload appears in the HTML — it does not; only a drawing of it does. `printed_ticket_page_test.dart` reads the rectangles back out of the drawn `<path>` and compares them **module for module** against the encoder's own output for the same string, so an optimisation of `QrSvg` that shifts a run by one module fails there rather than at a coach door. Walked end to end and proven: the printed pass was held to the conductor's reader, returned **VALID · Juste Bouiti · 11B · EC9FBP**, and the redemption row was read back out of Postgres with `synced_at` stamped — `docs/walkthroughs/agency-customer/the-ticket-you-print/`. Still open: the operator's uploaded logo, which needs `ticket_by_link()` to return the asset so it can be inlined as a `data:` URI. 9 unit · 8 Postgres |
| **No screen is a white field** | ✅ done | `05-design-system.md` §7.2 and §8.1. The walkthrough screenshots were the review: shot in order, twelve of the traveller's frames were a form floating on white with a thin coloured strip at the top and nothing at the bottom. Each one passed its widget tests. Two components close it. **`KActionBar`** — bottom-anchored, raised ground, the safe area, a line of context above the button (the price about to be paid, the note that a ticket works offline) and the primary action under it. It is the executed half of §8's *the primary action is always reachable by thumb*: a commitment sitting inline in the scroll is one a tall passenger list pushes under the fold. Six `findsOneWidget`s broke when the bar arrived — tests that had been finding a button only because it happened to be above the fold, which is the proof the bar was needed rather than a reason to revert it. **`KClosingScene`** — the illustrated foot. Short content on a tall handset leaves a band of bare cream between the last field and the bar; this fills it with a `KSceneArt` under a `brandPrimaryStrong` gradient, so a screen ends on the product rather than trailing off. It goes in a `SliverFillRemaining(hasScrollBody: false)` and is deliberately **not** a `LayoutBuilder`, which cannot report an intrinsic height to a sliver. Eight screens converted from `ListView` to `CustomScrollView` to take it. Sixteen traveller screens now end on the bar: sign-in, passengers, payment, confirm, waiting, checkout, result, hold, reserved, tickets, the ticket itself, the seat alert, settings, share, the travel choice, the cancel and the change. The rule is stated in the words it was given in, and **the check is looking, not asserting** — widget tests pass on a screen nobody would trust, so the walkthrough sequence is the gate: an all-white frame in it is the defect. The console's nav rail was fixed on the way past (36 px of overflow at 900 px, now a `SingleChildScrollView` over an `IntrinsicHeight` at `minHeight: constraints.maxHeight`, guarded by a source-level test that asserts the wrapping order). |
| **The driver's day — D0** | ✅ done | [`19-the-drivers-day.md`](19-the-drivers-day.md), and the server half of it. The conductor's list is a **span**: `GET /console/v1/boarding?from=&to=`, inclusive local dates, capped at 31 days, with `date=` still working because the handsets in the field are sideloaded APKs an agency updates when somebody drives to the office. One call and not seven — a week assembled from seven requests over a yard's signal finishes only if all seven do. Each row says whether the caller is rostered on that coach and as what, read from `departure_crew` as a **LEFT JOIN and never a filter**: a company that has not filled in a rota must not wake up to an empty screen, so what the roster changes is what a row *says*, not whether it is there. Where one person both drives and conducts the same run — which migration `0049` deliberately allows, because a three-coach company runs that way — driving is the job named, being the earlier commitment and the one a rota is planned around. And `GET /console/v1/me` answers a **name, a matricule and a company** where it answered a UUID. Three surfaces were rendering that id where a person's name belongs, and everybody who saw it read it as a bug. The matricule is an identifier and **not a credential** (ADR-0024) — written on the roster, printed on the manifest, stuck to the dashboard of the coach — and it is on the handset so somebody can check the device in their hand is signed in as them. The query is best-effort and its failure is a blank header rather than a failed sign-in: everything else on that response is what the client cannot work without. **The screens are D1–D5 and are not built** — the handset's Day/Week/Month filter, the console rota that would call the crew endpoint J3 shipped and nothing has ever called, the bell, the SSE thread, and the scan page's missing brand band. |
...
line of YAML. Seven items of **Phase 4** are built too: search is paged, which is the
...
turning a seat's state into a question with two answers; document expiry,
which is the first rule in this product that takes something away from an
operator without anybody deciding to; and onboarding that reviews itself,
which is the first that gives something — and only because the rule above it
can take it back. `09-roadmap.md` has the remaining
...
| **The platform split — P1** | ✅ done | `bel_platform` exists, is empty on purpose, and is wired into the pub workspace, nine dependent pubspecs and both hand-maintained fast-suite lists. Green first attempt: analyze clean, layers clean at 450 files, 610 pure-Dart tests, 1 370 API tests, and seven Flutter surfaces. Two things were checked rather than assumed — `dart test` on a package with no tests exits **79** alone but 0 alongside packages that have some, which is why both list call sites stay green; and `bel_domain` is deliberately **not** given the dependency yet, because it contains the platform types rather than importing them and gains the edge in P2a alongside its description change. Noticed and deliberately not fixed: `melos.yaml`'s `test:domain` omits `bel_client`, which CI's equivalent step includes |
| **The platform split — P2a** | ✅ done | Twenty files and 2 800-odd lines moved out of `bel_domain`, ten test files followed, 16 transport files repointed at `package:bel_platform`, both barrels rewritten, and one fenced transitional re-export so the slice reads as a move. **651 pure-Dart tests before, 651 after, same single pre-existing skip** — checked by stashing the change and re-running, not inferred. Four things the move found: the import rewriter missed bare same-directory imports (`import 'crypto_ports.dart';`) and left two ticketing files dangling; two files put `library;` after their doc comment, so prepending an import broke the directive order; `market_test.dart` turned out not to be a platform test at all, since two of its cases assert that *transport* carries no country assumption; and the transitional re-export announced itself immediately as an `unnecessary_import` on the first file to import both packages |
| **The platform other companies can join** | ⬜ specified | [`18-the-platform-other-companies-can-join.md`](18-the-platform-other-companies-can-join.md), 13 slices J1–J13. Written by walking the tree against the business's own description of the flow, so half of it is a list of what already exists and must not be rebuilt — the catalogue is already cross-operator, a driver who buys a ticket is already one `user_accounts` row with an `operator_staff` membership, and Tier 2 road progress already works. The finding is that **everything about selling a seat is built and almost nothing about the coach leaving is**: a departure has no crew, no state after `scheduled`, and no way to say it has gone — `departure_status` has carried `boarding`, `departed` and `arrived` since `0001` and only disruptions ever write that column. Four of the seven gaps are downstream of that one absence, which is why J3 (a crew) and J4 (the lifecycle) are the spine. Two slices close a distance between what `03-operator-lifecycle.md` promises and what the code enforces: **activation does not check for a verified collection account** although §1 says it does, and "suspension honours issued tickets" is a sentence rather than a schema guarantee. Rejects three things the description asked for: a country splash (ADR-0031), a driver id as a credential (ADR-0024 — it is an identifier, kept on the roster), and a live GPS map in the traveller app (ADR-0009's binary budget, and a moving dot is a promise the RN1's signal cannot keep). **[ADR-0033](adr/0033-the-web-traveller-surface.md)** answers the browser half: the whole journey, server-rendered by extending `infrastructure/web/`, **not** a Flutter web target — CanvasKit is ~2 MB before our own code, painted to a canvas that the cheap handset this surface exists for renders worse than HTML, and invisible to the search engine that is a marketplace's cheapest acquisition channel, against a follower-page budget of 50 KB and a boarding pass already rendered whole with no JavaScript at all. The same trade ADR-0022 made in the other direction, decided by the same binary budget |
| **The driver's day** | ⬜ specified | [`19-the-drivers-day.md`](19-the-drivers-day.md), 5 slices **D1–D5**. `apps/scanner` answers one question in under two seconds with the radio off and answers it well; it is silent on the three other moments in a driver's day — *which runs are mine this week*, *has anything changed*, *how do I tell the office* — and the answer to all three today is the same phone call, on a network that fails for four hours at a stretch on the RN1. **D1** puts a Day · Week · Month filter on the handset over the span endpoint that now exists, with the one rule that decides the slice: a row outside today is a plan row and **not pinnable**, because a manifest downloaded three weeks early is a list of tickets nobody has bought yet, and the conductor who pinned one arrives at the door missing half the coach. **D2** is the console rota — `POST /console/v1/departures/{id}/crew` has existed since J3 and **no screen has ever called it**, so a month of shifts can only be written with `curl`; behind `departure.manage` rather than `staff.manage`, because granting the driver role and putting somebody on Tuesday's 06:00 are different jobs done by different people, and the composite foreign key `departure_crew_is_staff` means rostering another company's employee **cannot be written** rather than merely being refused a screen. **D3/D4** are the bell and the thread: **server-sent events, not a WebSocket** — one-way push with an ordinary POST for the reply, surviving a proxy that mangles upgrades and resuming itself with `Last-Event-ID` after the four hours of nothing the RN1 costs; a message typed in a dead zone queues exactly as a boarding does and shows as *sending* rather than pretending it has gone, in a **third outbox** rather than a column on the boarding one, for the same reason the checkpoints are separate. **D5** is the last all-white screen in the product — the scan page, whose header is a bare `surfaceRaised` strip while every other screen wears the woven band; the camera below it is necessarily a black rectangle, which is exactly why the band matters, and whatever replaces the strip must not take a pixel from the camera or a millisecond from a scan. **All five are under one rule: the door comes first** — none of them may sit in the boarding path, hold a lock on the redemption log, or run a timer while a verdict is on screen |
| **Markets are context, not partitions** | ⬜ specified | [ADR-0031](adr/0031-markets-are-context.md), [`16-markets.md`](16-markets.md), 8 slices M1–M8. Found by asking why country was not the first filter, and the answer was worse than the question: **`market_code` is written on `operators`, `cities` and `user_accounts` and read exactly once in the entire tree** — `postgres_platform_console.dart:617`, to print it on an admin screen. `user_accounts.market_code` is read nowhere at all. The city-picker query has no market clause, so seeding Gabon would silently mix Libreville into a Brazzaville origin list — the same shape of defect as `TripDto.mode`. The decision rejects both country-as-a-first-step (it taxes every search for a rare case; country is context like language and currency, which this product already resolves rather than asks) and country-as-a-partition (there is no correct answer to which market a Brazzaville→Libreville coach is in, because the question is malformed — an *operator* has a market because it is licensed somewhere, a *city* has one because it is in a country, and a departure just connects two cities). `departures` has no market column and §5 of that document forbids adding one, because the absence is what makes cross-border work |
| **The platform split — P2b…P5** | ⬜ specified | [`15-platform-split.md`](15-platform-split.md), slices P1–P5. Blocking and shared, and the only one of these specified against a classifier **run over the tree** rather than an estimate: `bel_domain` splits 19 files / 2 802 lines platform against 19 files / 4 060 lines transport, **with zero overlap in public type names** — so P2a is a pure file move behind a one-commit transitional re-export. The internal graph holds exactly one forced decision (`identity/totp.dart` imports `ticketing/crypto_ports.dart`, so a platform file depends on a transport-filed one; `crypto_ports` moves). Of the **263 files importing `package:bel_domain`, 149 use platform symbols only** — 57% of the consumers of "the domain" never touch a seat. The scan also corrected itself: its first run reported 7 dead imports, which were an artefact of a regex that did not match `abstract interface class`, and §11 ships the fixed script so the numbers can be re-derived rather than believed |
...
10. ~~**The section builder edits sections, not cells.**~~ **Mostly closed.**
   Tapping a seat on the preview takes it out of service and tapping it again
   gives it back; doors, lavatories, galleys, exits, luggage, the driver, the
   cockpit and stairs are placed by coordinate; sections move forward and back
   without being retyped. Blocking is a *subtraction*, not a shorter section —
   the seat keeps its label so the manifest, the ticket and the conductor's
   count still agree with the physical coach, and no seat row is created for
...
0d. **The vitrine had no address.** `GET /public/v1/operators/{code}` has
   ...
   operator pressed save on a screen called *votre vitrine* and had nowhere to
   send anybody. `blt.cg/o/<code>` is now a page, rendered whole on the server
   because it exists to be shared and the crawler building the WhatsApp
   preview card runs no JavaScript. Indexable, unlike the follower page and
   for the opposite reason. 20 tests, 11 smoke checks.
...
`Ed25519TicketIssuer.development()` builds a signer from 32 bytes generated by
`(i * 7 + 13) & 0xff` — a fixed seed, on purpose, so a ticket in yesterday's
screenshot still scans today (ADR-0020). Its own comment said production keys
are generated in and never leave the KMS, and that swapping the line was the
whole change. Nothing swapped it. `Services.resolve` handed that issuer to the
Postgres composition exactly as it handed it to the fakes.

**What that meant.** A deployment against a real database would have signed
real tickets with 32 bytes printed in a public repository. The forgery is not
subtle — generate the same seed, sign any payload you like, and the scanner
verifies it against the public key it fetched at login and goes green in front
of the conductor. Every anti-replay control downstream of the signature is
about screenshots of *real* tickets; none of them looks at a ticket that was
never sold.

**The seed is configuration now, and its absence is not survivable.**
`TICKETS__SIGNINGSEED` is 32 bytes of base64. A process talking to a real
database refuses to start without one. Three refusals are worth naming
individually:

A seed of the wrong length is refused rather than padded or truncated. A value
somebody pasted half of is a key nobody can reproduce, and every ticket signed
with it stops verifying on the day somebody notices and fixes it.

A passphrase is refused by name rather than decoded into whatever bytes it
happens to make. The variable is base64, not a password, and the error says
which.

The development seed is refused **as a value**. It is in the source, so
copying it into a secret manager is the same accident wearing a costume, and
the only way to catch that is to compare against it and say so.

**Production is the default.** The one escape is `BEL__ENV=development`, which
`infra/dev/.env` sets and no deployment does. This direction is the whole
point: a variable that has to be *set* to make a process strict is a variable
somebody forgets on the day it matters, and the failure mode of forgetting
this one is silent for the lifetime of the deployment.

The demo seeder resolves its signer the same way instead of calling the
development factory unconditionally, because a seeded world whose tickets do
not verify against its own scanner is a demo that fails at the coach door —
the one place it exists to rehearse. `tool/demo.sh` declares development only
when it is pointed at a local stack, and says in its own comments what to set
when it is pointed at a staging box.

**The KMS is still not built, and ADR-0007 now says so.** The seed is a
Kubernetes secret held in the process, which is weaker than that ADR
describes: cluster access reads it, and the key exists in memory rather than
behind an API that only ever signs. Moving to a KMS is one file and the shape
of `Ed25519TicketSigner`; the wire format has carried a key id since the first
ticket, so rotation will never need the app to change.

**What it cost:** one config file, 11 tests, one changed adapter factory, one
changed seeder, and a fixed test seed exported by `tool/integration.sh` — that
suite now exercises the configured path rather than the fallback, which is
right, because it is the suite that talks to a real database.
